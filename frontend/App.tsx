...
import { AppStep, OrderDetails, ServiceCategory, LocationInfo, PackageDetails, Store, MenuItem, CartItem, UserProfile, PaymentMethod, DeliveryTier, PriceBreakdown, MovingDetails, PropertyType, BusinessLogisticsDetails, VehicleType, TripType, OrderHistoryItem, OrderStatus, TrackingData } from './types.ts';
import { SERVICE_TYPES, MOCK_RESTAURANTS, MOCK_MENUS } from './constants.ts';
import Header from './components/Header.tsx';
import StepIndicator from './components/StepIndicator.tsx';
import LocationInput from './components/LocationInput.tsx';
import Notifications from './components/Notifications.tsx';
import * as whatsappService from './services/whatsappService.ts';
...
import { ChevronRightIcon, ArrowLeftIcon, StarIcon, ShoppingCartIcon, PackageIcon, CalendarIcon, MotorcycleIcon, PhoneIcon, ChatBubbleIcon, UserIcon, MailIcon, CameraIcon, CreditCardIcon, CashIcon, DevicePhoneMobileIcon, GoogleIcon, BanknotesIcon, BuildingOfficeIcon, SmsIcon, WhatsAppIcon, HomeIcon, BuildingStorefrontIcon, TruckIcon, RouteOptimizationIcon, BellIcon, MapPinIcon, ArrowRightOnRectangleIcon, FoodIcon, GroceryIcon, PharmacyIcon, HeavyItemIcon, DocumentIcon, ClipboardDocumentListIcon, SpinnerIcon, ExclamationTriangleIcon } from './components/icons.tsx';
...
const LoginStep: React.FC<{ 
    ...
    useEffect(() => {
        // The reCAPTCHA is now initialized on-demand by the firebaseService.
        // We only need to ensure it's cleaned up when the login view is no longer visible.
        return () => {
            ...
            <div className="mt-6 text-sm text-center">
                <p className="text-slate-500">
                    ...
                    <button 
                        ...
                        className="font-medium text-red-700 hover:text-red-600 disabled:text-slate-400 disabled:cursor-not-allowed"
                    >
...
const App: React.FC = () => {
    ...
    const [isLoading, setIsLoading] = useState(false); // For phone login
    ...
    // Effect to manage the OTP resend cooldown timer
    useEffect(() => {
        let timer: number;
        if (resendCooldown > 0) {
    ...
    const sendOtp = async (phone: string): Promise<boolean> => {
        ...
        try {
            ...
            if (error.code === 'auth/invalid-phone-number') {
                friendlyMessage = t('login.error.otpInvalidNumber');
            } else if (error.code === 'auth/too-many-requests') {
                friendlyMessage = t('login.error.otpTooManyRequests');
    ...
    const handlePhoneNumberSubmit = async (phone: string) => {
        ...
        if (success) {
            ...
            setResendCooldown(30); // Start cooldown on first send
        }
    };

    const handleResendOtp = async () => {
        if (resendCooldown > 0) return;
        const success = await sendOtp(loginPhoneNumber);
        if (success) {
    ...
    const renderStep = () => {
        const orderFlowSteps = [
            AppStep.LOCATION_INPUT, AppStep.PACKAGE_DETAILS, AppStep.MOVING_DETAILS, 
            AppStep.BUSINESS_LOGISTICS_DETAILS, AppStep.CONFIRMATION
        ];

        const isOrderFlow = orderFlowSteps.includes(appStep);

        const currentFlowSteps = useMemo(() => {
            const steps = [{ step: AppStep.LOCATION_INPUT, label: t('stepIndicator.location') }];
            if (orderDetails.serviceType === ServiceCategory.Package || orderDetails.serviceType === ServiceCategory.Document || orderDetails.serviceType === ServiceCategory.Heavy) {
                steps.push({ step: AppStep.PACKAGE_DETAILS, label: t('stepIndicator.details') });
            }
            steps.push({ step: AppStep.CONFIRMATION, label: t('stepIndicator.confirm') });
            return steps;
        }, [orderDetails.serviceType, t]);
        ...
        switch (appStep) {
            ...
            case AppStep.OTP_VERIFICATION:
                return <OtpStep 
                    ...
                    isError={!!apiError && !otpNotification}
                    isLoading={isLoading}
            ...
            case AppStep.CONFIRMATION:
                 const confirmationBackStep = (orderDetails.serviceType === ServiceCategory.Package || orderDetails.serviceType === ServiceCategory.Document || orderDetails.serviceType === ServiceCategory.Heavy)
